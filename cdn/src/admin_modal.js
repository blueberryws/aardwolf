...
(() => {
    ...
    // Define autonomous custom element (Safari-friendly)
    if (!customElements.get("admin-modal")) {
        customElements.define("admin-modal", AdminModal);
    }

    // Global factory for classical scripts (preferred creation path)
    function modalBuilder() {
        return document.createElement("admin-modal");
    }
